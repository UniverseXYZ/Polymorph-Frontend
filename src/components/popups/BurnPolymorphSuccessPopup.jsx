...
const BurnPolymorphSuccessPopup = ({onClose, characters}) => {
  ...
  return (
    <div className='burn--polymorph--success--popup'>
      ...
      <p className='desc'>{`You have successfully burnt your old polymorph${characters.length > 1 ? 's' : ''} and minted a new one${characters.length > 1 ? 's' : ''}`}</p>
      {loading ? 
        ...
        <div className='batch--polymorphs'>
          {loading ?
            ...
            <Slider {...settings}>
              {characters.map(c => {
                return (
                  <div className='polymorph--img' key={c.id}>
        ...
        <div className='single--polymorph'>
          {loading ?
            ...
            <div className='polymorph--img'>
              <img src={characters[0].polymorphImg} alt='Polymorph' />
            </div>
          }
        </div>
      }
      <div className='actions'>
        <Button className='light-button' onClick={onClose}>Burn More</Button>
        <Button className='light-border-button' onClick={() => router.push('/my-polymorphs')}>My Polymorphs</Button>
      </div>
    </div>
  )
}
